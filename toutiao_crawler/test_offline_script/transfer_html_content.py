...
def get_samples():
	...
	print("共有{0}个样本".format(len(results)))
...
def filter(id_list, htmls):
	...
	for i in range(len(id_list)):
		...
		id = id_list[i]
		html = htmls[i]
		content_clean = BeautifulSoup(html, 'lxml').get_text()
		revise_news(id, content_clean)


def revise_news(id, content_clean):
	conn = pymysql.connect(host='116.62.106.69', port=3306, user='datag', passwd='yjkdatag', db='news_crawler',
						   charset='utf8')
	cursor = conn.cursor()

	sql = "update toutiao_app_combine_unique_20170623 set content = %s where id = %s"
	cursor.execute(sql, (content_clean, id))
	conn.commit()
