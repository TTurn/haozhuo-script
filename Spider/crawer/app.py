        ...
        for json_content in json_result:
            ...
            if (title not in result) & (title<>""):
                if json.loads(json_content["content"]).has_key("display_url"):
                    ...
                    print display_url
                    print json.loads(json_content["content"])["article_url"]
                    try:
                        result2 = opener.open(display_url)
                        html= result2.read()
                        soup = BeautifulSoup(html)
                        pstr = ""
                        # print soup.find_all(class_="time")[0].text
                        ...
                        if len(soup.find_all(id="article-main")) > 0:
                            ...
                            image_list = str(json.loads(json_content["content"])["image_list"]) if json.loads(
                                ...
                                "image_list") else "";
                            content_list = [title, keywords, abstract, content, source, article_url, display_url, image_list,contents,create_time]
                            result_list = []
                            result.append(title)
